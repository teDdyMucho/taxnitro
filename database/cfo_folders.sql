-- ============================================================
-- CFO ADVISORY — folder tables
-- New "CFO" category (3rd client service). Six document folders:
--   cfo_contracts, cfo_invoices, cfo_additional_docs
--   Monthly Reporting (nested on the client side):
--     cfo_mr_required_info, cfo_mr_client_review, cfo_mr_final_statements
--
-- Each table matches the columns the app reads/writes (see src/db/documents.ts):
-- Document interface + createDocumentRecord (incl. uploaded_by_role).
-- Full RLS: client sees/edits/deletes own; staff/admin manage all.
...
begin
  foreach t in array cfo_tables loop
